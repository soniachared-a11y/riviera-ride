...
const BookingForm = () => {
  ...
  const handleGetQuote = async () => {
    ...
    try {
      const result = await calculateRoute(watchedDepart, watchedArrivee);
      setQuote(result);
      setStep('details');
    } catch (error) {
      console.error('Erreur calcul:', error);
      setErrorMessage("Impossible de calculer l'itinéraire. Vérifiez les adresses.");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <button
          ...
          disabled={isLoading}
          className="bg-white text-black px-6 py-3 rounded font-light transition-all duration-300 hover:bg-black hover:text-white hover:border-white border border-transparent text-sm tracking-widest disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
          {isLoading ? (
            <>
