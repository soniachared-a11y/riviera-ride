...
const SCHEMA = {
  ...
  "geo": { "@type": "GeoCoordinates", "latitude": 43.4356, "longitude": 5.2218 },
  "areaServed": [
...
const TaxiAeroportMarignane = () => {
  useEffect(() => {
    ...
    s1.type = 'application/ld+json'; s1.id = 'schema-aeroport-business';
    s1.textContent = JSON.stringify(SCHEMA);
    document.head.appendChild(s1);
    const s2 = document.createElement('script');
    s2.type = 'application/ld+json'; s2.id = 'schema-aeroport-faq';
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '80px 24px 60px' }}>
        <div style={{ marginBottom: '48px' }}>
          ...
          <h1 style={{ fontSize: 'clamp(28px, 4vw, 48px)', fontWeight: 300, letterSpacing: '1px', lineHeight: 1.2, marginBottom: '24px' }}>
            ...
            <span style={{ display: 'block', color: 'rgba(255,255,255,0.6)', fontSize: '0.6em', fontWeight: 200, marginTop: '8px' }}>Marignane — Transfert depuis Aix-en-Provence</span>
          </h1>
          <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: '17px', lineHeight: 1.7, maxWidth: '600px' }}>
            Service de taxi et VTC entre Aix-en-Provence et l'Aéroport Marseille-Provence (Marignane). Chauffeur Tesla disponible <strong style={{ color: '#fff' }}>24h/24 7j/7</strong>, tarif fixe garanti, suivi de vol en temps réel, accueil nominatif au Hall des arrivées.
          </p>
        </div>
        ...
        <div style={{ marginBottom: '40px' }}>
          ...
          <ul style={{ listStyle: 'none', padding: 0, display: 'grid', gap: '12px' }}>
            {['Suivi des vols en temps réel — adaptation automatique en cas de retard','Accueil au Hall des arrivées avec panneau nominatif','Véhicule Tesla — wifi, chargeurs, climatisation','Tarif fixe annoncé à la réservation, sans surprise','Disponible 24h/24 — y compris vols de nuit et tôt le matin'].map((item) => (
